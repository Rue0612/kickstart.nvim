...
-- Keep signcolumn on by default"none",
...
--  Also Ctrl+neio is already what I use for moving around so it would be more ackward Cmd+Ctrl+something....
--  (And I use niri multiwindow so I dont even use those i guess)
--
...
-- Terminal keymaps to make it easyer to open a terminal in the directory that I am working ( As I preffer using my window manager for tabs. there is keymaps for opening on nvim tho)
...
require('lazy').setup({
  ...
  { -- Snacks (replaces Telescope)
    -- Picker(Telescope) + Smooth Scroll
    -- Yes it was vibecoded sory, I liked the telescope config but prefered snack pick so just asked that.
    -- Hate to say that because ironnically would be faster just to do myself, AI cant even do that.
    'folke/snacks.nvim',
    lazy = false,
    priority = 1000,
    ...
    keys = {
      ...
      { '<leader>ss', function() Snacks.picker.pickers() end, desc = '[S]earch and [S]elect Picker' },
      { '<leader>sw', function() Snacks.picker.pick 'grep_word' end, desc = '[S]earch current [W]ord' },
      { '<leader>sg', function() Snacks.picker.grep { live = true } end, desc = '[S]earch by [G]rep' },
        ...
        opts = {
          notification = {
            window = {
              border = 'rounded',
              winblend = 0,
              -- So fidget has this strange bug that when you have an transparent terminal it shows a black thingy when this is set to bigger than 0. Yes, I would preffer to be able to read text behind it, but the black thing is worse in my eyes
            },
          },
        },
      },
